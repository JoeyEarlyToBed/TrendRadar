...
class AppContext:
    ...
    def render_html(
        ...
        freq_file = getattr(self.config, "frequency_file", "config/frequency_words.txt")
        role_summary = get_role_summary(stats, frequency_file=freq_file)

        # 构建 word → role 映射
        word_role_map = {}
        role_stats_map = classify_stats_by_role(stats, frequency_file=freq_file)
        for role, role_stats_list in role_stats_map.items():
            for s in role_stats_list:
                w = s.get("word", "")
                if w:
                    word_role_map[w] = role

        report_data["_word_role_map"] = word_role_map
