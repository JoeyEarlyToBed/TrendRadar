...
def render_html_content(
    ...
    *,
    ...
    role_summary: Optional[List[Dict]] = None,
) -> str:
    """渲染HTML内容
    ...
    <head>
        ...
        <style>
            * { box-sizing: border-box; }
            ...
            .role-tab-btn:hover { background: #eef0f2; color: #374151; }
            .role-tab-btn.active {
                background: white;
                color: #111827;
                box-shadow: 0 1px 4px rgba(0,0,0,0.08);
            }
            .role-tab-btn .rt-count {
                font-size: 11px;
                background: rgba(0,0,0,0.08);
                padding: 1px 6px;
                border-radius: 10px;
            }
            .role-tab-btn.active .rt-count { opacity: 0.85; }
            .role-tab-btn .rt-label {
                font-size: 10px;
                color: #9ca3af;
                display: none;
            }
            body.wide-mode .role-tab-btn .rt-label { display: inline; }

            body.dark-mode .role-tab-bar { border-bottom-color: #334155; }
            body.dark-mode .role-tab-btn {
                background: #1e293b;
                color: #94a3b8;
            }
            body.dark-mode .role-tab-btn:hover {
                background: #334155;
                color: #e2e8f0;
            }
            body.dark-mode .role-tab-btn.active {
                background: #1e293b;
                color: #f1f5f9;
            ...
            .role-badge.role-CEO { background: #fde8e8; color: #c0392b; }
            .role-badge.role-CTO { background: #dbeafe; color: #1d4ed8; }
            .role-badge.role-CDO { background: #d1fae5; color: #059669; }
            .role-badge.role-QM  { background: #fef3c7; color: #d97706; }

            body.dark-mode .role-badge.role-CEO { background: rgba(231,76,60,0.15); color: #e74c3c; }
            body.dark-mode .role-badge.role-CTO { background: rgba(41,128,185,0.15); color: #5dade2; }
            body.dark-mode .role-badge.role-CDO { background: rgba(39,174,96,0.15); color: #58d68d; }
            body.dark-mode .role-badge.role-QM  { background: rgba(243,156,18,0.15); color: #f5b041; }
    ...
    # ── 角色仪表盘 ──
    role_tabs_html = ""
    word_role_map = report_data.get("_word_role_map", {})  # {word: role}
    if role_summary:
        role_tabs_html = '<div class="role-tab-bar">'
        role_tabs_html += '<button class="role-tab-btn active" data-role="all">全部角色</button>'
        for rs in role_summary:
            role = html_escape(rs['role'])
            name = html_escape(rs['name'])
            color = html_escape(rs['color'])
            count = rs.get('title_count', 0)
            role_tabs_html += (
                f'<button class="role-tab-btn" data-role="{role}" '
                f'style="border-left-color:{color}">'
                f'{name}<span class="rt-count">{count}</span>'
                f'</button>'
            )
        role_tabs_html += '</div>'
    ...
    if report_data["stats"]:
        ...
        for i, stat in enumerate(report_data["stats"], 1):
            ...
            stats_html += f"""
                <div class="word-group" data-tab-index="{i - 1}"{role_attr}>
                        {role_badge}
    ...
    if stats_html:
        stats_html = f"""
                <div class="hotlist-section">{role_tabs_html}{tab_bar_html}{stats_html}
                </div>"""
    ...
    html += """
        ...
        <script>
            ...
            // ===== 角色仪表盘 Tab 切换 =====
            (function() {
                var roleBar = document.querySelector('.role-tab-bar');
                if (!roleBar) return;
                var roleBtns = roleBar.querySelectorAll('.role-tab-btn');
                var allGroups = document.querySelectorAll('.word-group[data-tab-index]');

                function filterByRole(role) {
                    // 先高亮 keyword tab 的 "全部"
                    var keywordBar = document.querySelector('.tab-bar');
                    if (keywordBar) {
                        var keywordAll = keywordBar.querySelector('[data-tab-index="all"]');
                        if (keywordAll) {
                            keywordBar.querySelectorAll('.tab-btn').forEach(function(b) { b.classList.remove('active'); });
                            keywordAll.classList.add('active');
                        }
                    }
                    // 触发 keyword tab 切换函数
                    if (window.switchKeywordTab) {
                        window.switchKeywordTab('all');
                    }
                    ...
                    allGroups.forEach(function(g) {
                        if (role === 'all') {
                            ...
                            var groupRole = g.getAttribute('data-role') || '';
                            g.style.display = (groupRole === role) ? '' : 'none';
                        }
                    });
                }
                ...
                roleBtns.forEach(function(btn) {
                    btn.addEventListener('click', function() {
                        ...
                        filterByRole(btn.getAttribute('data-role'));
                    });
                });
            })();
        </script>
